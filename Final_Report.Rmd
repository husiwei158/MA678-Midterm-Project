...
###i. Data source
The raw data has 7620 competition from 242 game clubs and 15 Leagues. The raw data has 6 different datasets. 
Here are two dataset i used in my project:
...
ggplot(data = LOL.all,aes(x = gamelength, y = bResult))+ geom_jitter(alpha = 0.3) +  geom_smooth(method = "glm", method.args = list(family = "binomial"),formula = y~x) + xlab("Gamelength") + ylab("Win Rate of Blue Team") + ggtitle("How Gamelength affect the Competition (Plot.1)") + theme(axis.title = element_text(size = 13))
...
par(mfrow=c(1,2))
plot(fitted(model),resid(model)); abline(h=0,lty = 3)
